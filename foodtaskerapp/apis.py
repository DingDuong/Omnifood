    ...
    if request.method == "POST":
        ...
        if len(order_details) > 0:
            ...
            charge = stripe.Charge.create(
                amount = round(order_total * 100), # Amount in cents
                currency = "usd",
                source = stripe_token,
                description = "Omnifood Order"
            )
...
def customer_driver_location(request):
    ...
    #Get driver's location related to customer's order
...
def driver_pick_order(request):
    if request.method == "POST":
        ...
        try:
            ...
            return JsonResponse({"status": "failed", "error": "This order has been picked by another"})
